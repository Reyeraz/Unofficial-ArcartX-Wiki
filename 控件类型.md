...
### 文字 `Text`
  ...
  * 文字组件用于显示纯文字内容。
  * 使用 &(颜色代码) 以显示原版的文字颜色, 比如  `texts: ~&7示例`
  * 使用 §#(HEX颜色代码) 以显示16进制颜色, 比如 `texts: ~§#FFFFFF示例`
  * **独有属性:**
...
slotType| 槽位类型| Backpack| `Container` 容器槽位，比如箱子界面的上方的那些格子 `Backpack`背包槽位
`Extra`自定义拓展槽位  
id| 槽位ID| 0| 除了拓展槽位是字符串，其余两种都是数字ID  
itemScale| 物品缩放比例| 1.0| 物品缩放比例  
lock| 锁定点击| false| 是否锁定点击事件，锁定后无法点击槽位物品（仅客户端）  
itemEffect| 物品额外渲染| | 物品额外渲染效果，填写物品效果名称，具体请查看ItemEffect使用说明  
### 多层血条 `BossBar`
